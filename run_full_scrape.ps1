...
if ($RunAll -or $LocalOnly) {
    ...
    & $PythonExe tools/ingest_all_local.py
...
if ($RunAll -or $Dedupe) {
    ...
    if ($DB) {
        & $PythonExe tools/deduplicate_shelters.py --merge
    } else {
        & $PythonExe tools/deduplicate_shelters.py
    }
...
if ($RunAll) {
    ...
    & $PythonExe tools/split_by_state.py
